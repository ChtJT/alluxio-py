...
# -------------------------------
# 1) TEXT: minimal tokenizer + tiny classifier
# -------------------------------
class SimpleTokenizer:
    """
    极简 whitespace tokenizer，返回与 HF 兼容的 dict(keys: input_ids, attention_mask)
    """
    ...
    def __call__(self, text, truncation=True, padding=False, return_tensors=None):
        ...
        ids = np.array([self.vocab.get(t, self.unk_id) for t in toks], dtype=np.int32)
...
class TinyTextClassifier(nn.Module):
    ...
    词嵌入 + 均值池化 + 线性分类
    """
    def __init__(self, vocab_size, dim=32, num_labels=2):
        super().__init__()
...
def test_image_training(tmp_path):
    ...
    # 2) dataset / dataloader（把图片统一到 32x32；这里原图已是 32）
